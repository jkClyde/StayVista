...
const PropertyFilters = ({ initialFilters = {} }) => {
  ...
  const updateURL = (newFilters) => {
    ...
    // Add guest params
    if (newFilters.adults > 0) {
      params.set('adults', String(newFilters.adults));
    }

    if (newFilters.children > 0) {
      params.set('children', String(newFilters.children));
    }

    if (newFilters.infants > 0) {
      params.set('infants', String(newFilters.infants));
    }

    // Preserve checkIn and checkOut if they exist in current URL
  ...
  const handleGuestChange = (guestType, increment) => {
    const currentValue = filters[guestType];
    const minValue = guestType === 'adults' ? 1 : 0;
    const newValue = Math.max(minValue, currentValue + increment);
  ...
  const GuestCounter = ({ label, subtitle, value, guestType, minValue = 0 }) => (
    <div className='flex items-center justify-between py-3'>
      ...
      <div className='flex items-center gap-3'>
        <button
          type='button'
          onClick={() => handleGuestChange(guestType, -1)}
        ...
        <div className='border-b border-gray-200 pb-6'>
          ...
          >
            <h3 className='font-semibold text-gray-900'>
              Guests <span className='text-sm font-normal text-gray-600'>({totalGuests})</span>
            </h3>
